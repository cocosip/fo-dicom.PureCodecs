...
{
    [Fact]
    public void Openjph_irreversible_htj2k_fixture_decodes_with_pure_decoder()
    {
        ...
        var frame = new Jpeg2000HtFrameCodec().DecodeFrame(decoded, codestream);

        Assert.Equal(decoded.UncompressedFrameSize, frame.Length);
