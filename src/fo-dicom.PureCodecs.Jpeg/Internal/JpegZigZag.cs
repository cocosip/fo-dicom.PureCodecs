...
namespace FellowOakDicom.PureCodecs.Jpeg.Internal
{
    public static class JpegZigZag
    {
        ...
        public static double[] ToZigZag(JpegBlock8x8 block)
        {
            ...
            CopyToZigZag(block, values);
            return values;
        }

        internal static void CopyToZigZag(JpegBlock8x8 block, double[] values)
        {
            if (block == null)
            {
                throw new ArgumentNullException(nameof(block));
        ...
        internal static void SetFromZigZag(JpegBlock8x8 block, int index, double value)
        {
            if (block == null)
            {
                throw new ArgumentNullException(nameof(block));
            ...
            {
                throw JpegMarkerReader.CreateException("JPEG zigzag coefficient index is outside the valid range.");
            }

            block[Order[index]] = value;
        }
    }
}
