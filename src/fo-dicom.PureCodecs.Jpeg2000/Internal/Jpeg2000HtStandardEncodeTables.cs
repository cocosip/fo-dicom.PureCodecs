...
{
    internal static class Jpeg2000HtStandardEncodeTables
    {
        internal static readonly int[] VlcTable0Source =
        {
        ...
        internal static readonly int[] VlcTable1Source =
        {
        ...
        public static readonly ushort[] VlcTable0 = BuildVlcTable(VlcTable0Source);

        public static readonly ushort[] VlcTable1 = BuildVlcTable(VlcTable1Source);

        private static ushort[] BuildVlcTable(int[] source)
        {
            ...
            for (var i = 0; i < table.Length; i++)
            {
                ...
                var rho = (i >> 4) & 0xF;
                var emb = i & 0xF;
                if (((emb & rho) != emb) || (rho == 0 && context == 0))
                {
                    continue;
                }
                ...
                if (emb != 0)
                {
                    var bestEmbeddingBits = -1;
                    for (var offset = 0; offset < source.Length; offset += 7)
                    {
                        if (source[offset] != context || source[offset + 1] != rho || source[offset + 2] != 1)
                        {
                            continue;
                        }

                        var embedding = source[offset + 3];
                        if ((emb & embedding) != source[offset + 4])
                        {
                            continue;
                        }

                        var embeddingBits = PopulationCount(embedding);
                        if (embeddingBits >= bestEmbeddingBits)
                        {
                            bestOffset = offset;
                            bestEmbeddingBits = embeddingBits;
                        }
                    }
                }
                else
                {
                    for (var offset = 0; offset < source.Length; offset += 7)
                    {
                        if (source[offset] == context && source[offset + 1] == rho && source[offset + 2] == 0)
                        {
                            bestOffset = offset;
                            break;
                        }
                ...
                if (bestOffset < 0)
                {
                    throw Jpeg2000Binary.CreateException($"HTJ2K standard VLC encode tuple is not available for context {context}, rho 0x{rho:X}, embedding 0x{emb:X}.");
                }

                table[i] = (ushort)((source[bestOffset + 5] << 8) | (source[bestOffset + 6] << 4) | source[bestOffset + 3]);
            }
