...
- `JpegCodecParams` provides quality, predictor, point transform, and `ConvertColorspaceToRGB`, with color conversion enabled by default for Process 1 and Process 2/4. Lossless encode writes predictor and point transform to SOS, and lossless decode reads both values from the codestream. Non-zero point transforms are covered in both Pure-to-Native and Native-to-Pure directions.
