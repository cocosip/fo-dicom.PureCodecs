...
HTJ2K release validation does not require `fo-dicom.Codecs` Native
interoperability. Its required gates are the managed round-trip matrix, standard
HT cleanup-pass vectors, and local OpenJPH-derived codestream fixtures. Existing
Native/OpenJPH compatibility tests are supplementary evidence only and are not
part of the nine-format Native interoperability gate used for the other codec
families.
...
- HTJ2K managed round-trips, standard HT block vectors, and local
  OpenJPH-derived codestream fixtures for lossless, lossless RPCL, and lossy
  transfer syntaxes; Native interoperability is not a required HTJ2K gate.
