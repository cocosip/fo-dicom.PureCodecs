...
The current pure C# HTJ2K path implements a project-managed HT block
experiment used by the local codec tests: MEL event coding, a validated HT VLC
table subset based on OpenJPH Annex C table entries, MagSgn coding, cleanup
quad scanning, and three-segment block assembly. The DICOM adapter currently
stores that data in a project-managed `PHTJ` tile payload, so it is not a
standard OpenJPH/`fo-dicom.Codecs` compatible HTJ2K codestream.
...
This is not public HTJ2K interoperability. The independent vector covers the
first standard block path; broader standard codestream import, non-initial
quad rows, refinement passes, and replacing the `PHTJ` tile payload with
standard Part 15 HT block data remain compatibility work.
...
The current HTJ2K encoder uses the same envelope shape but stores a private
`PHTJ` payload in the tile data section, so HTJ2K must remain unregistered by
`PureTranscoderManager` until that payload is replaced with standard Part 15
HT block data.
...
- Required DICOM compression tag checks for JPEG 2000 lossless and JPEG 2000
  lossy.
- Managed exceptions for invalid JPEG 2000 codestreams and internal HTJ2K
  payload experiments.
